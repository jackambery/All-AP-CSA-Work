...
//#2b
   public boolean endsWith2Stars (String s) {
      ...
      char last = s.charAt(s.length() - 1);
      char secLast = s.charAt(s.length() - 2);
      int total = s.length();
      
      if (last == '*' && secLast == '*' && total >= 2) {
         return true;
      }
      else {
         return false;
...
//#4b
   public String modifyDate(String dateStr) {
      ...
      String month = dateStr.substring(0, firstDash);
      if (month.length() == 1) {
         month = "0" + month;
      }
      
      String day = dateStr.substring(firstDash + 1, secondDash);
      if (day.length() == 1) {
         day = "0" + day;
      }
